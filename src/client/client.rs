...
use crate::merkletree::tree::{ OFFSET_ONE, OFFSET_TWO};
use crate::merkletree::tree::FastMerkleNode;
...
fn get_root_hash_from_leaves(leaves: Vec<Hash>) ->FastMerkleNode{
   ...
   let mut leaves = leaves;
    ...
    pub async fn handle_file_hash(mut form: FormData)->Result<impl warp::Reply, Rejection>{
        ...
        let root_hash = format!("{:?}",get_root_hash_from_leaves(file_hash_list).value.as_bytes().to_vec());
   ...
   pub async fn handle_verify(mut form: warp::multipart::FormData) -> Result<impl warp::Reply, Infallible> {
