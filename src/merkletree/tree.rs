...
mod tests {
    ...
    fn test_get_file_hashes()
    {
        ...
        let file_hash_list : Vec<Vec<u8>> = serde_json::from_str(test_file_hash_list_string).unwrap();

        let file_list = get_file_list(TEST_DIR);
        let file_hashes = get_file_hashes(file_list).iter().map(|h|h.as_bytes().to_vec()).collect::<Vec<_>>();
